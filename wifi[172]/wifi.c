...
//#define F_CPU 16000000		/* Define CPU Frequency e.g. here its Ext. 12MHz */
...
void wifi_init()
{
	...
	LCD_printAt(0, "USART_DONE");
	while (!ESP8266_Begin());
	LCD_printAt(0, "Wifi is ok!");
	ESP8266_WIFIMode(BOTH_STATION_AND_ACCESPOINT);        /* 3 = Both (AP and STA) */
	LCD_printAt(0, "Wifi Mode OK");
	ESP8266_ConnectionMode(SINGLE);                    /* 0 = Single; 1 = Multi */
	LCD_printAt(0, "Wifi CONN OK");
	ESP8266_ApplicationMode(NORMAL);                    /* 0 = Normal Mode; 1 = Transperant Mode */
	LCD_printAt(0, "Wifi APP OK");

	if (ESP8266_connected() == ESP8266_NOT_CONNECTED_TO_AP)
		ESP8266_JoinAccessPoint(SSID, PASSWORD);
	LCD_printAt(0, "Wifi Connected");
	_delay_ms(10);
	ESP8266_Start(0, DOMAIN, PORT);
	LCD_printAt(0, "Wifi Init OK");
	_delay_ms(10);
...
void wifi_send(char* buff)
{
		...
		sprintf(_buffer, "GET /update?api_key=%s&field3=%s", API_WRITE_KEY, buff);
		ESP8266_Send(_buffer);
}


// DEMO - uri for testing purposes
// int main(void)
// {
// 	char _buffer[150];
// 	uint8_t Connect_Status;
...
// 		#ifdef SEND_DEMO
// 		memset(_buffer, 0, 150);
// 		sprintf(_buffer, "GET /update?api_key=%s&field2=%d", API_WRITE_KEY, 1);
// 		ESP8266_Send(_buffer);
// 		_delay_ms(15000);
// 		memset(_buffer, 0, 150);
//
...
// 		ESP8266_Send(_buffer);
// 		memset(_buffer, 0, 150);
// 		sprintf(_buffer, "GET /update?api_key=%s&field4=%d", API_WRITE_KEY, Sample++);
// 		ESP8266_Send(_buffer);
// 		memset(_buffer, 0, 150);
// 		sprintf(_buffer, "GET /update?api_key=%s&field5=%d", API_WRITE_KEY, Sample++);
// 		ESP8266_Send(_buffer);
